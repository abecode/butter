...
define( [ "util/lang", "util/uri", "util/keys", "util/mediatypes", "editor/editor",
 "util/time", "util/dragndrop", "text!layouts/media-editor.html","util/xhr" ],
  ...
  function addElements( data, el ) {
    ...
    el.querySelector( ".mg-starts-at" ).innerHTML = Time.toTimecode( data.start_time,2);
    if ( data.type === "HTML5" ) {
  ...
  Editor.register( "media-editor", null, function( rootElement, butter ) {
    ...
    xhr.get( railsApp + "/api/v1/events/"+butter.project.eventId+"/artist/"+butter.project.artistId, loadAvailableMedia );
    for ( var key in clips ) {
      if ( clips.hasOwnProperty( key ) ) {
        clip = clips[ key ];
